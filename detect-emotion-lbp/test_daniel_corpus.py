...
class detect_emotion(object):
    ...
    pd = None
    pl = None
    emotions = ("boredom", "engagement", "excitement", "focus", "interesting", "relax")
    ...
    def predict(self, image):
        ...
        resize = cv2.resize(gray, (480, 640))  # Imagenes estandarizadas a 500 pixeles
        clahe_image = self.clahe.apply(resize)
        result, img = self.__get_image__(clahe_image)
        if result:
            y = self.model.predict(np.array(img))
            returnValue = (True, self.emotions[y[0]])
    ...
    @staticmethod
    def processImage(item, clahe, detect, data, labels, emotion):
        ...
        if result == "error" or result == False:
            ...
            data.append(landmarks_vectorised)  # append image array to training data list
            labels.append(detect.emotions.index(emotion))
    ...
    def get_files(emotion):  # Define function to get file list, randomly shuffle it and split 80/20
        files = glob.glob(r"D:\version2\%s\*.jpeg" % emotion)
    ...
    def crossValidation(self, cv=10):
        ...
        # Matriz de confusión
        yTrue = map(lambda index: self.emotions[index], self.pl)
        yPred = map(lambda index: self.emotions[index], self.model.predict(self.pd))
        cm = confusion_matrix(yTrue, yPred, self.emotions)
        print(cm)

#sdetector = detect_emotion.create_model_training()
